 ...
 function showAllMessagesWithChosenUser(selectedUserId){
    $.ajax({
          ...
          success: function(response) {
            ...
            changeHeaderToChatWithUsername(selectedUserId);
...
function scrollDownMessageContainer(){
   var chatHistory = document.getElementById("messagesContainer");
   chatHistory.scrollTop = chatHistory.scrollHeight;
}

function changeHeaderToChatWithUsername(userId){
    $.ajax({
          type: "POST",
          url: "/findUserById",
          data: { userId: userId },
          success: function(response) {
            ...
            header.innerHTML = "Chat With " + response;
          },
          error: function (xhr, status, error) {
               console.error("Error during AJAX request. Status: " + status + ", Error: " + error);
               console.log("XHR object:", xhr);
